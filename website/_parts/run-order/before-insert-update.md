...
2. **Handlers are adapted.** `<ctx>Handlers()` is called, and each handler in the list is adapted by its role. A class with both roles becomes a Populator; a class with neither is dropped.
...
5. **Each handler runs, in list order.**
   ...
   - The DML guard checks that the handler ran no DML and published no platform event, and throws if it did.
   - After a Populator, parents are queried for any lookup it pointed at a parent that is not loaded yet.
6. **Nothing to commit.** The shared unit of work is committed, but it is always empty here: no handler in a before context receives a unit.
...
The platform then saves the records. All of this happens once per trigger run, and the platform runs a trigger once per chunk of up to 200 records.
