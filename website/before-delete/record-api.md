...
- **Lookups hold only the Id.** `((Contact) record.getOldSObject()).Account` is null. Declare a [PriorParentQuery](/before-delete/add-ons/prior-parent-query) and read `getOldParent(Contact.AccountId)`.
