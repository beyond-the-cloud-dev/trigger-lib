...
In after contexts a Writer does not run DML. It registers writes on [`TriggerHandler.UnitOfWork`](/api/unit-of-work), and the library commits them in bulk: one statement per operation and object type.
...
| ContinueOnError, without OwnUnitOfWork | a separate unit, set up like the default one |
| neither | the default unit of work of the run |

The default unit of work is `new DML().combineOnDuplicate().systemMode().withoutSharing().identifier('triggerUow')`. Only Writers get a unit. A Dispatcher gets none.

## When It Commits {#when-it-commits}

- **The default unit of work commits once per run,** after the last handler.
- **Any other unit commits right after its Writer,** when at least one record qualified. Later handlers can query its writes.
- **A failed default commit fails the save.** ContinueOnError does not apply, and `TriggerOrchestrator.Logger` never sees it.
...
- **Duplicates merge.** In the default and separate units, a second `toUpdate` or `toDelete` of the same Id merges into the first. Later fields win.
...
- **A plain `new DML()` differs from the default unit of work.** It runs in user mode and throws on a duplicate registration.
